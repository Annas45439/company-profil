...
const Shuffle = ({
  ...
  useGSAP(
    ...
    { dependencies: [text, fontsLoaded, shuffleDirection, duration, maxDelay, ease, shuffleTimes, animationMode, loop, loopDelay, stagger, colorFrom, colorTo, triggerOnce, triggerOnHover, scrollTriggerStart], revertOnUpdate: true }
  );

  const Tag = tag;

  return (
    <Tag ref={ref} className={`shuffle-text ${className}`} style={{ textAlign, visibility: ready ? 'visible' : 'hidden', ...style }}>
      {text}
    </Tag>
  );
};
